...
const AdminHeader = ({ currentTab, onTabChange }: AdminHeaderProps) => {
  ...
  return (
    <header className="text-zinc-200 flex items-center justify-center py-10">
      <div className="flex items-center justify-between gap-3 bg-zinc-900/50 backdrop-blur-md p-0.5 rounded-2xl border border-zinc-800 shadow-2xl">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={cn(
                "py-2 px-3 rounded-xl transition-all duration-300 group relative flex items-center gap-2",
                isActive 
            ...
            >
              ...
              {/* {isActive && (
                <span className="text-xs font-medium pr-1 animate-in fade-in slide-in-from-left-2 duration-300">
                  {tab.label}
                </span>
              )} */}
              {!isActive && (
